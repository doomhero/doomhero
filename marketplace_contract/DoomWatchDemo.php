<?php
	...
	$marketContractAddress = "0xfcf1126b873005b24d6c2cebe395751c02c67a97";
	...
	echo 'balance of owner: '.(balanceOf($tokenContract, $tokenContractAddress, $ownerAddress)).'<br>';
	echo 'balance of seller: '.(balanceOf($tokenContract, $tokenContractAddress, $sellerAddress)).'<br>';
	echo 'balance of buyer: '.(balanceOf($tokenContract, $tokenContractAddress, $buyerAddress)).'<br>';
	echo 'balance of market: '.(balanceOf($tokenContract, $tokenContractAddress, $marketContractAddress)).'<br>';

	echo '>> step 1: mint a token: '.$tokenId.'<br>';
	$mintData = '0x'.$nftContract->at($nftContractAddress)->getData('mint', $sellerAddress, $tokenId);
	$tx = sendTransactionAndWaitSuccess($eth, $utils, $ownerAddress, $nftContractAddress, $ownerPrik, $mintData);
	...
	$tx = sendTransactionAndWaitSuccess($eth, $utils, $sellerAddress, $nftContractAddress, $sellerPriky, $appoveData);
	echo "tx hash: " . ($tx -> transactionHash) . '<br>';
	...
	$tx = sendTransactionAndWaitSuccess($eth, $utils, $sellerAddress, $marketContractAddress, $sellerPriky, $createData);
	echo "tx hash: " . ($tx -> transactionHash) . '<br>';
	...
	echo '>> step5: buyer 购买<br>';
	$buyData = '0x'.$marketContract->at($marketContractAddress)->getData('buy', $nftContractAddress, $tokenId, $sellAmount);
	$tx = sendTransactionAndWaitSuccess($eth, $utils, $buyerAddress, $marketContractAddress, $buyerPriky, $buyData);
	echo "tx hash: " . ($tx -> transactionHash) . '<br>';
	$owner = ownerOf($nftContract, $nftContractAddress, $tokenId);
	echo "owner of ".$tokenId.': '.$owner. '<br>';
	...
	echo '>> step6: 提取到 owner<br>';
	$reclaimData = '0x'.$marketContract->at($marketContractAddress)->getData('reclaimToken');
	$tx = sendTransactionAndWaitSuccess($eth, $utils, $ownerAddress, $marketContractAddress, $ownerPrik, $reclaimData);
	echo "tx hash: " . ($tx -> transactionHash) . '<br>';
	echo 'balance of owner: '.(balanceOf($tokenContract, $tokenContractAddress, $ownerAddress)).'<br>';
	echo 'balance of market: '.(balanceOf($tokenContract, $tokenContractAddress, $marketContractAddress)).'<br>';
	 ...
	 * @return 返回交易hash
	 */
	function sendTransactionAndWaitSuccess($eth, $utils, $from, $contractAddress, $priK, $functionData){
		try {
			$txHash = sendTransaction($eth, $utils, $from, $contractAddress, $priK, $functionData);
			$txReceipt = null;
			for ($i=0; $i <= 10; $i++) {
	 ...
	 * @return 返回交易hash
	 */
	function sendTransaction($eth, $utils, $from, $contractAddress, $priK, $functionData){
		// 获取 nonce 和 price
		$nonce = 0;
		$gasPrice = 0;
		...
		// 估算 gasLimit
		$transactionInfo = ['from' => $from,'to' => $contractAddress,'data' => $functionData];
